...
def initialize_particles(number_of_particles):
    ...
    personal_best_values = np.array([fitness(x1, x2) for x1, x2 in swarm])
...
def update_particles(inertion, lbest_coefficient, gbest_coefficient):
    ...
    if use_modified_version.get():
        fi = lbest_coefficient + gbest_coefficient
        chi = 2 / abs(2 - fi - np.sqrt(fi * (fi - 4)))
    else:
        chi = 1
    ...
    for i in range(len(swarm)):
        ...
        current_fitness = fitness(swarm[i][0], swarm[i][1])
...
def run_algorithm():
    ...
    function_value_label.config(text=f"Значение функции: {function(*global_best_position):.5f}")
...
inertion_input.grid(row=0, column=1)
...
lbest_coefficient_input.grid(row=1, column=1)
...
gbest_coefficient_input.grid(row=2, column=1)
...
particles_input.grid(row=3, column=1)
...
def set_iterations(value):
    iterations_input.delete(0, tk.END)
    iterations_input.insert(0, str(value))

buttons_frame = tk.Frame(controls_frame)
buttons_frame.grid(row=1, column=0, columnspan=2, pady=5)
tk.Button(buttons_frame, text="1", width=10, command=lambda: set_iterations(1)).pack(side="left", padx=0)
tk.Button(buttons_frame, text="10", width=10, command=lambda: set_iterations(10)).pack(side="left", padx=0)
tk.Button(buttons_frame, text="100", width=10, command=lambda: set_iterations(100)).pack(side="left", padx=0)
tk.Button(buttons_frame, text="1000", width=10, command=lambda: set_iterations(1000)).pack(side="left", padx=0)
...
toggle_mode_button.grid(row=3, column=0, sticky="w")
